...
describe('ARRAY()', () => {
  ...
  it('empty should verify', helper.mochaAsync(async () => {
    const promise = ARRAY(undefined, {
      noEmptyArrays: true
    }).empty(false).validate([]);

    await helper.throw(promise, 'Array is empty.');

    const result = await ARRAY(undefined, {
      noEmptyArrays: true
    }).empty(true).validate([]);
    result.should.deepEqual([]);
  }));
  ...
  it('duplicate items should fail', helper.mochaAsync(async () => {
    let result = await helper.shouldThrow(async () => ARRAY().unique(true).validate(['a', 'b', 'a']));
    result.should.equal('Values must be unique.');

    result = await helper.shouldThrow(async () => ARRAY(OBJECT({
      id: INTEGER()
