...
describe('Date()', () => {
  describe('required()', () => {
    ...
    it('undefined should verify', async () => {
      const actual = await validator.Date()
        .required(false)
        .validate();
  ...
  describe('default()', () => {
    it('invalid default value should throw', async () => {
      ...
      await helper.throw(func, `Not a valid date. Must match format '${helper.DATE_FORMAT}'.`);
    });
