...
describe('Regex()', () => {
  ...
  describe('required()', () => {
    it('undefined should throw', async () => {
      const promise = validator.Regex(/[A-Z]/)
        .required(true)
        .validate();
    ...
    it('undefined should verify', async () => {
      const actual = await validator.Regex(/[A-Z]/)
        .required(false)
        .validate();
  ...
  describe('validate()', () => {
    it('invalid type should fail', async () => {
      for (const value of [1, true]) {
        await helper.throw(validator.Regex(/[A-Z]/).validate(value), `Must be string but is ${typeof value}.`);
      }
    });
    ...
    it('invalid value should fail', async () => {
      await helper.throw(validator.Regex(/[A-Z]/).validate('abc'), 'Value does not match regular expression.');
    });
  });

  describe('min()', () => {
    it('invalid length should fail', async () => {
      await helper.throw(validator.Regex(/[A-Z]/).min(5).validate('ABC'), `Must have at least 5 characters.`);
    });
  ...
  describe('max()', () => {
    it('invalid length should fail', async () => {
      await helper.throw(validator.Regex(/[A-Z]/).max(3).validate('ABCD'), 'Must have at most 3 characters.');
    });
  ...
  describe('length()', () => {
    it('invalid length should fail', async () => {
      await helper.throw(validator.Regex(/[A-Z]/).length(3).validate('ABCD'), 'Must have exactly 3 characters.');
    });
  ...
  describe('default()', () => {
    it('should throw', async () => {
      await helper.throw(() => validator.Regex(/[A-Z]/).default(1234), 'Must be string.');
    });

    it('should return default value', async () => {
      const result = await validator.Regex(/[A-Z]/).default('ABC').validate();
      result.should.equal('ABC');
    });

    it('should return value', async () => {
      const result = await validator.Regex(/[A-Z]/).default('ABC').validate('DEF');
  ...
  describe('empty()', () => {
    it('should throw', async () => {
      await helper.throw(validator.Regex(/[A-Z]/).empty(false).validate(''), 'String is empty.');
    });

    it('should verify', async () => {
      const result = await validator.Regex(/[A-Z]/).empty(true).validate('');
      result.should.equal('');
    });
  });
  ...
  describe('message()', () => {
    it('should return custom message', async () => {
      ...
      await helper.throw(validator.Regex(/[A-Z]/).empty(false).message(message).validate('abc'), message);
    });
  });

  describe('toObject()', () => {
    it('should return object full description', async () => {
      const actual = validator.Regex(/A-Z/)
        .name('name')
