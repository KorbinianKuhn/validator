...
const get = (language, messages, schemaType, error, ...replaceValues) => {
  ...
  let message = _.clone(
    _.get(MESSAGES, `${schemaType}.${error}.${messages}.${language}`)
  );
...
exports.required = (language, messages, value) =>
  get(language, messages, 'generic', 'required', value);

exports.wrongType = (language, messages, schemaType, value) =>
  get(language, messages, 'generic', 'wrong_type', schemaType, value);
